...
def processFlat(F):
    ...
    """
    # C-NMF params
    niter = config["niters"]  # Iterations for the MF and clustering

    # Preprocess to obtain features, times, and input boundary indeces
    #F = self._preprocess()

    # Normalize
    F = U.normalize(F, norm_type=config["norm_feats"])
    #F = utils.normalize(F, norm_type="min_max")
    #print(F.shape)
    if F.shape[0] >= config["h"]:
        # Median filter
        F = median_filter(F, M=config["h"])
        #plt.imshow(F.T, interpolation="nearest", aspect="auto"); plt.show()

        # Find the boundary indices and labels using matrix factorization
        est_idxs, est_labels = get_segmentation(F.T, config["rank"], config["R"],config["rank_labels"], config["R_labels"],
            niter=config["niters"], bound_idxs=None, in_labels=None)
